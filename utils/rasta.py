...
def rastaplp(x, fs=16000, win_time=0.025, hop_time=0.01, dorasta=True, modelorder=8):
...
def powspec(x, fs=16000, window_time=0.025, hop_time=0.01, dither=1):
...
def fft2barkmx(fft_length, fs, nfilts=0, band_width=1, min_freq=0, max_freq=0):
    ...
    for i in range(int(nfilts)):
        ...
        wts[i, 0: int(fft_length / 2) + 1] = np.power(
            10, np.minimum(0, np.divide(np.minimum(hif, np.multiply(-2.5, lof)), band_width)))
    return wts
